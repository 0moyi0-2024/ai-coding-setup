...
[Run]
; 安装完成后询问是否立即运行
; 先以当前交互用户启动，DSH-一键安装.exe 自身再按需请求 UAC，避免安装器身份
; 把 PowerShell 7/WSL 初始化绑定到错误的管理员用户配置文件。
Filename: "{app}\{#MyAppExeName}"; Description: "立即开始安装 DSH"; Flags: postinstall nowait skipifsilent shellexec runascurrentuser
