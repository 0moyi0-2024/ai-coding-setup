...
#   --merge               追加 JD 支持（默认；Codex 默认切到 JD，Claude 默认仍走火山）
...
在已有 Claude Code 和 Codex 安装旁追加京东（JD LLM Gateway）独立配置。
不带模式参数时默认追加 JD 支持，把普通 Codex 默认切到 JD，并隐藏输入 token。
...
选项:
  ...
  --merge               追加 JD 配置（默认）：写入 .codex/jd.config.toml，为 Claude
                        生成 claude-jd 启动器，并把普通 Codex 默认切到 JD
  --claude-only         只生成 Claude Code 配置
  --codex-only          只生成 Codex 配置
  --no-probe            不访问网关，直接使用内置回退模型
...
默认追加模式不会修改 Claude settings.json；普通 claude 仍默认通过 CCR 使用火山。
JD 会注册到 CCR，并成为普通 codex 的默认 provider；也可以显式使用:
  claude-jd
  codex --profile jd
USAGE
}
...
configure_jd_ccr_provider() {
  local -a model_names=("$@")
  if ((DRY_RUN)); then
    log '[dry-run] 将把 JD provider 追加到 CCR；CCR 默认火山路由保持不变'
    return 0
  fi
  ccr_integration_available || {
    warn '未检测到基础安装器管理的 CCR；JD profile 和 claude-jd 已生成，但尚未注册到 CCR'
    return 0
  }
  if ((NO_SAVE_TOKEN)); then
    warn '--no-save-token 不会把 JD token 写入 CCR；JD profile 仍可使用当前环境变量'
    return 0
  fi
  require_command curl
  start_ccr_if_needed || die 'CCR 服务无法启动；JD profile 已生成，但 JD provider 尚未注册到 CCR'

  local models config_response config request response
  models=$(printf '%s\n' "${model_names[@]}" | jq -R . | jq -s .)
  config_response=$(jd_ccr_rpc '{"method":"getConfig","args":[]}')
  [[ "$(jq -r '.ok // false' <<<"${config_response}")" == true ]] ||
    die '无法读取 CCR 配置；JD profile 已生成，但 JD provider 尚未注册到 CCR'
  config=$(build_jd_ccr_config "${config_response}" "${models}")
  request=$(jq -cn --argjson config "${config}" \
    '{method:"saveConfig",args:[$config,{applyProfile:false}]}')
  response=$(jd_ccr_rpc "${request}")
  [[ "$(jq -r '.ok // false' <<<"${response}")" == true ]] ||
    die "无法把 JD provider 写入 CCR: $(jq -r '.error.message // "未知错误"' <<<"${response}")"
  log "已把 ${#model_names[@]} 个已验证 JD 模型注册到 CCR；默认火山路由保持不变"
}
...
build_jd_provider_registration() {
  local -a models=("$@")
  local models_toml='' quoted_model base_url_toml auth_line
  local first=1 model
  for model in "${models[@]}"; do
...
is_generated_jd_provider() {
  ...
  awk -v expected_base="${CODEX_BASE_URL}" '
    ...
    in_provider && ($0 == "env_key = \"JD_GATEWAY_TOKEN\"" ||
      $0 ~ /^http_headers = \{ Authorization = \"Bearer /) { auth_ok=1 }
    in_provider && $0 == "wire_api = \"responses\"" { wire_ok=1 }
    END { exit !(name_ok && base_ok && auth_ok && wire_ok) }
  ' "${target}"
}
...
register_jd_codex_provider() {
  ...
  if ((DRY_RUN)); then
    log "[dry-run] 将在 ${main_config} 注册 JD provider"
    return 0
  fi
  ...
  # Refresh an older generated JD table, but keep an intentionally custom
  # provider definition untouched.
  ...
  rm -f -- "${tmp}"
  log "已在 ${main_config} 注册 JD provider"
}

without_codex_top_level_defaults() {
  local target=$1
  [[ -f "${target}" ]] || return 0
  awk '
    $0 == "# BEGIN JD gateway defaults" { skip=1; next }
    $0 == "# END JD gateway defaults" { skip=0; next }
    skip { next }
    !in_table && /^[[:space:]]*\[/ { in_table=1 }
    !in_table && /^[[:space:]]*(model|model_provider|model_catalog_json|model_reasoning_effort|plan_mode_reasoning_effort)[[:space:]]*=/ { next }
    { lines[++count]=$0 }
    END {
      first=1
...
set_jd_codex_defaults() {
  local -a models=("$@")
  local main_config model model_toml catalog_toml dir tmp final target_mode
  main_config=$(codex_main_config_path)
  model=$(select_codex_model "${models[@]}")
  [[ -n "${model}" ]] || die '无法选择 JD 默认 Codex 模型'
  [[ -n "${CODEX_CATALOG_FILE}" ]] || die 'JD 模型 catalog 路径为空'

  if ((DRY_RUN)); then
    log "[dry-run] 将把 ${main_config} 的默认 Codex provider 切换为 JD (${model})"
    return 0
  fi

  dir=$(dirname "${main_config}")
  mkdir -p "${dir}"
  tmp=$(mktemp "${dir}/.jd-defaults.XXXXXXXX")
  final="${tmp}.final"
  without_codex_top_level_defaults "${main_config}" >"${tmp}"
  model_toml=$(toml_string "${model}")
  catalog_toml=$(toml_string "${CODEX_CATALOG_FILE}")
  {
    printf '# BEGIN JD gateway defaults\n'
    printf 'model = %s\n' "${model_toml}"
    printf 'model_provider = "jd"\n'
    printf 'model_catalog_json = %s\n' "${catalog_toml}"
    printf 'model_reasoning_effort = "xhigh"\n'
    printf 'plan_mode_reasoning_effort = "max"\n'
    printf '# END JD gateway defaults\n'
    if [[ -s "${tmp}" ]]; then
      printf '\n'
      cat "${tmp}"
    fi
  ...
  mv -f -- "${final}" "${main_config}"
  rm -f -- "${tmp}"
  log "已把普通 Codex 默认网关切换为 JD (${model})"
}
...
main() {
  ...
  if (( ! CLAUDE_ONLY )); then
    ...
    if ((MERGE)); then
      ...
      log 'Codex JD profile 和动态模型目录已生成'
...
HINT
  ...
  if ((MERGE && !CLAUDE_ONLY && !OUTPUT_DIR_SET)); then
    ...
    set_jd_codex_defaults "${codex_models[@]}"
  fi
