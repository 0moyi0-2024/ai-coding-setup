...
我选了 [PR 37](https://dongdev.jd.com/codex/xLLM_AI/xllm/merges/37)——DeepSeek V4 在 xLLM 框架上的首个算子绑定 PR，作为走读对象。读完这篇文章你会搞清楚三件事：

1. **xLLM 的整体架构**长什么样？每个目录干啥的？
...
xLLM 是一个大模型推理框架，采用**双层架构**设计：

```
┌─────────────────────────────────────────────────────┐
│               HTTP / gRPC 请求                       │
├─────────────────────────────────────────────────────┤
│         C++ Serving Framework（服务层）               │
│                                                      │
│  api_service → Scheduler → Engine → Worker → Executor│
│       │                                  │           │
│       ▼                                  ▼           │
│  Tokenizer / Chat Template  模型 forward / logits    │
│  Continuous Batching         KV Cache 管理           │
│  PD 分离调度                 采样 / 投机解码          │
├─────────────────────────────────────────────────────┤
│         Python Model Execution（模型层）              │
│                                                      │
│  models/ (PyCausalLM) ←→ python/models/ (nn.Module) │
├─────────────────────────────────────────────────────┤
│         Kernel / Hardware（算子层）                   │
│                                                      │
│  C++ kernels (TORCH_LIBRARY) ←→ Python kernels_npu  │
│  AscendC / ACLNN / TileLang   torch.ops.xllm_ops.*  │
└─────────────────────────────────────────────────────┘
```
...
| 模型定义（Attention、MoE 结构） | **Python** | 新模型迭代快，改 Python 比改 C++ 效率高十倍 |
| 底层算子 | **C++ + Python** | 追求极致性能用 C++ AscendC，灵活组合用 Python JIT |

简单说：**性能敏感的用 C++，迭代频繁的用 Python。** 社区开发者想接入新模型，只需要写 `nn.Module` + `load_weights()`，不用碰 C++ 代码。
...
```
xllm/
├── api_service/        # HTTP API 处理层（chat/completions 等接口）
...
为 DeepSeek V4 的 DSA 稀疏注意力和 Hash-MoE 新算子，建立从 **C++ AscendC 内核 → Python 模型层** 的完整调用链路。
...
| 新特性 | 需要的算子 | 做什么 |
|---|---|---|
| **DSA 稀疏注意力** | `compressor`、`sparse_attn_sharedkv`、`quant_lightning_indexer` | 先选出关键 KV 块，再做两阶段稀疏注意力 |
| **HyperConnection** | `hc_pre`、`hc_post` | 多层特征融合的预处理/后处理 |
| **Hash-MoE 路由** | `moe_gating_top_k_hash` | 用 token ID 哈希替代可学习 gate 选专家 |
| **W8A8 融合量化** | `dequant_swiglu_quant`、`rms_norm_dynamic_quant` | RMSNorm/激活函数与 INT8 量化融合 |
...
```cpp
// xllm/core/kernels/npu/npu_ops_library.cpp
TORCH_LIBRARY(xllm_ops, m) {
  m.def(
      "compressor(Tensor x, Tensor wkv, Tensor wgate, "
      ...
      "Tensor? cu_seqlens, Tensor? seqused, Tensor? start_pos, "
      "int rope_head_dim, int cmp_ratio, int coff, float norm_eps, "
      "int rotary_mode, bool enable_grad) "
      "-> (Tensor, Tensor, Tensor, Tensor, Tensor)");
}
```

这行代码的作用是告诉 PyTorch dispatcher：**我这有个叫 `compressor` 的算子，接收这些参数，返回 5 个 Tensor。** 相当于在 PyTorch 的算子注册表里登记了一下。
...
```python
def _compressor_fake(x, wkv, wgate, kv_state, score_state, ...):
    del wkv, wgate  # Fake 模式不关心值，只算 shape
    return (
        x.new_empty(kv_state.shape[:2], x.shape[1], x.shape[2], dtype=x.dtype),
        ...
    )
...
```python
def compressor(x, wkv, wgate, kv_state, score_state, ...):
    """NSA-style KV pooling compressor."""
    return torch.ops.xllm_ops.compressor(
        x, wkv, wgate, kv_state, score_state, ...
    )
```
...
| 直接调的问题 | 薄包装解决 |
|---|---|
| 要自己处理 NPU 特有的 reshape 细节 | 内部做了 reshape/view |
| 参数顺序全是位置参数，魔法数字多 | 提供具名参数+文档 |
...
cmp_kv, wkv_proj, softmax_res, norm_x, norm_rstd = kernels_npu.compressor(
    x, wkv, wgate, kv_state, score_state, ...
)
```
...
```
模型层 (python/models/deepseek_v4.py)
  │  kernels_npu.compressor(...)
  ▼
Python 薄包装 (python/kernels_npu/dsa.py)
  │  torch.ops.xllm_ops.compressor(...)
  ▼
PyTorch Dispatcher
  │  发现 Tensor 在 NPU 上 → 路由到 PrivateUse1
  ▼
C++ 实现 (core/kernels/npu/xllm_ops/compressor.cpp)
  │  xllm::kernel::npu::compressor(...)
  ▼
AscendC Kernel (ACLNN / TileLang)
  │  在 NPU 芯片上真正执行
  ▼
结果 Tensor 返回 Python
```
...
def grouped_moe_with_selected_experts(hidden_states, topk_weights, topk_ids, ...):
    # ① 路由：按 topk_ids 把 token 分发到对应专家
    expanded_hidden, expanded_row_idx, expert_tokens, _ = \
        torch_npu.npu_moe_init_routing_v2(hidden_states, topk_ids, ...)
    ...
    intermediate_i8, intermediate_scale = dequant_swiglu_quant(gemm1_out, ...)
...
这个函数被注册为 `torch.library.custom_op`，所以它也能被 `torch.compile` 追踪和优化。**组合逻辑放 Python 是因为 MoE 编排变化快，但每个子算子已经是 C++ 优化过的，性能不受影响。**
...
TEST_F(NpuOpsTest, DsaCompressor) {
  ...
  auto [cmp_kv, wkv, sm, nx, nrstd] = torch::xllm_ops::compressor(...);
...
def test_grouped_moe_orchestration(monkeypatch):
    ...
    monkeypatch.setattr(torch_npu, "npu_moe_init_routing_v2",
                        lambda **kw: calls.append("init_routing") or (mock_expanded, ...))
    # ... mock 其他算子
...
| 惰性 Python 模块注册 | 避免静态初始化顺序灾难（SIOF） |
...
AscendC C++ 内核（已有）
       ↓ PR 37 补齐的链路
TORCH_LIBRARY 注册 → FakeTensor 契约 → Python 薄包装 → 包级导出
       ↓ 产出
模型层可以直接 from xllm.python import kernels_npu 调用新算子
       ↓ 后续 PR
python/models/deepseek_v4.py 用这些算子组装完整的 DSA + Hash-MoE 前向
```

理解了这个 PR，你就理解了 xLLM 最核心的设计哲学：**C++ 兜底性能，Python 保证灵活性。** 后续无论加什么新算子、接什么新模型，都是同一个套路。

> 📎 仓库地址：`dongdev.jd.com/codex/xLLM_AI/xllm`
> 
